...
ggplot(loop, aes(x=`corr_minus_2AP`, y=`corr_FI`))+ geom_point(size=3, alpha=1, show.legend = FALSE)+
  ...
  theme_minimal()
ggplot(loop, aes(y=`corr_minus_2AP`,`corr_plus_2AP`, x=`colony`, color=`colony`, fill=`colony`)) +
  geom_bar(stat="identity", position=position_dodge(), show.legend = FALSE, alpha = 0.5) +
  labs(title="L4 Repression and Expression", x="Colony", y="Fluorescence")+
  theme_minimal()
```
#Anchor
```{r}
...
ggplot(n6p4stem, aes(x=`corr_minus_2AP`, y=`corr_FI`))+ geom_point(size=3, alpha=1, show.legend = FALSE)+
  ...
  theme_minimal()
#uhhhhh? idk if this is what I
ggplot(n6p4stem, aes(y=`corr_minus_2AP`,`corr_plus_2AP`, x=`colony`, color=`colony`, fill=`colony`)) +
  geom_bar(stat="identity", position=position_dodge(), show.legend = FALSE, alpha = 0.5) +
  labs(title="L4 Repression and Expression", x="Colony", y="Fluorescence")+
  theme_minimal()
```
...
#CONTROLS
```{r}
Controls <-read.csv("~/Downloads/Expression_platform_project/expression_platform_files/analysis/output_files/Controls/controls_summary_table.csv")
colnames(Controls) <- c("colony","-2AP", "-2AP_SE","+2AP", "+2AP_SE", "FI","corr_minus_2AP", "corr_plus_2AP","corr_FI")

ggplot(Controls, aes(x=`corr_minus_2AP`, y=`corr_FI`))+ geom_point(size=3, alpha=1, show.legend = FALSE)+
...
#TOEHOLD
```{r}
Toehold <- rep("L4",nrow(loop))
L4_TH <- cbind(loop, Toehold)
Toehold <- rep("P4",nrow(n6p4stem))
P4_TH <- cbind(n6p4stem, Toehold)
Toehold <- rep("NH5", nrow(NH5))
NH5_TH <- cbind(NH5, Toehold)
toehold_plot <- rbind(L4_TH, P4_TH, NH5_TH)
ggplot(toehold_plot, aes(x=`corr_minus_2AP`, y=`corr_FI`, color=Toehold))+ geom_point(size=3, alpha=0.6, show.legend = TRUE)+
  labs(title="Toehold Libraries Repression vs Activity", x="-2AP Fluorescence", y="Fold Induction")+
  theme_minimal()
```
...
label <-rep("library",nrow(AE))
TH <- rep("no",nrow(AE))
AE <- cbind(AE,library,label,TH)
library <- rep("GATEWAY",nrow(GATE))
label <-rep("library",nrow(GATE))
TH <- rep("no",nrow(GATE))
GATE <- cbind(GATE,library, label,TH)
library <- rep("LOOP",nrow(loop))
label <-rep("library",nrow(loop))
TH <- rep("L4",nrow(loop))
loop <- cbind(loop,library,label,TH)
library <- rep("ANCHOR",nrow(anchor))
label <-rep("library",nrow(anchor))
TH <- rep("no",nrow(anchor))
anchor <- cbind(anchor,library,label,TH)
library <- rep("N6P4STEM",nrow(n6p4stem))
label <-rep("library",nrow(n6p4stem))
TH <- rep("P4",nrow(n6p4stem))
n6p4stem <- cbind(n6p4stem,library,label,TH)
library <- rep("P3",nrow(P3))
label <-rep("library",nrow(P3))
TH <- rep("no",nrow(P3))
P3 <- cbind(P3, library,label,TH)
library <- rep("P1EXCHANGE",nrow(P1exchange))
label <-rep("library",nrow(P1exchange))
TH <- rep("no",nrow(P1exchange))
P1exchange <- cbind(P1exchange, library,label,TH)
library <- rep("del27_WT", nrow(del27_WT))
label <-rep("del27_WT",nrow(del27_WT))
TH <- rep("del27_WT",nrow(del27_WT))
del27_WT <- cbind(del27_WT, library,label,TH)
library <- rep("WT", nrow(WT))
label <-rep("WT",nrow(WT))
TH <- rep("WT",nrow(WT))
WT <- cbind(WT, library,label,TH)
library <- rep("NH5", nrow(NH5))
label <-rep("NH5",nrow(NH5))
TH <- rep("NH5",nrow(NH5))
NH5 <- cbind(NH5, library,label,TH)
plotdf <- rbind(GATE[,c(1:9,17:19)],AE[,c(1:9,16:18)],loop,anchor,n6p4stem,P3, P1exchange, del27_WT, WT ,NH5)
#plotdf$TH <- as.factor(plotdf$TH) 
#plotdf$TH <- factor(TH,level=c('NH5','WT','del27_WT','L4','P4','no'))
legend_title <- "Variant"
#highlighting only the controls
ggplot(plotdf, aes(x=`corr_minus_2AP`, y=`corr_FI`, color=factor(label,level=c('NH5','WT','del27_WT','library'))))+ geom_point(size=3, alpha=1, show.legend = TRUE)+
  labs(title="All Libraries Repression vs Activity", x="-2AP Fluorescence", y="Fold Induction")+
  theme_minimal() +
  scale_color_manual(legend_title, values = c("#00FFFF", "#FF00FF", "#FF0066", "#CCCCCC" ))
#highlighting controls and the toehold libraries 
ggplot(plotdf, aes(x=`corr_minus_2AP`, y=`corr_FI`, color=factor(TH,level=c('NH5','WT','del27_WT','L4','P4','no'))))+ geom_point(size=3, alpha=0.5, show.legend = TRUE)+
  labs(title="All Libraries Repression vs Activity", x="-2AP Fluorescence", y="Fold Induction")+
  theme_minimal() +
  scale_color_manual(legend_title, values = c("#00FFFF", "#FF00FF", "#FF0066","#3346FF","#21952A", "#CCCCCC" ))
...
megabin <- rbind(AEbins,GATEbins, ANCHbins, N6P4bins, LOOPbins, P1EXbins, P3bins)
